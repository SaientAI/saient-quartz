...
fn q4k_row_batched(x: &[f32], in_dim: usize, n: usize, row: &[u8], acc: &mut [f32]) {
    ...
    for b in 0..bpr {
        ...
        for _ in 0..4 {
            ...
            for l in 0..32 { wv[o + l] = d1 * (qs[q_off + l] & 0x0F) as f32 - m1; }
            o += 32;
            for l in 0..32 { wv[o + l] = d2 * (qs[q_off + l] >> 4) as f32 - m2; }
            o += 32; q_off += 32; is += 2;
        }
        let base = b * 256;
        for t in 0..n {
...
fn q6k_row_batched(x: &[f32], in_dim: usize, n: usize, row: &[u8], acc: &mut [f32]) {
    ...
    for b in 0..bpr {
        ...
        for _ in 0..2 {
            ...
            ql_off += 64; qh_off += 32; sc_off += 8; y_off += 128;
        }
        let base = b * 256;
        for t in 0..n {
            let xt = &x[t * in_dim + base..t * in_dim + base + 256];
...
pub fn gemm(x: &[f32], data: &[u8], ggml_type: u32, in_dim: usize, out_dim: usize, n: usize) -> Vec<f32> {
    if n == 1 {
        return gemv(x, data, ggml_type, in_dim, out_dim);
    }
    let row_bytes = crate::gguf::ggml_type_size(ggml_type, in_dim);
    let avx = cpu_has_avx2();
    let cols: Vec<Vec<f32>> = (0..out_dim).into_par_iter().map(|j| {
        let row = &data[j * row_bytes..(j + 1) * row_bytes];
        let mut acc = vec![0.0f32; n];
        match ggml_type {
            12 => if avx { unsafe { q4k_batched_avx2(x, in_dim, n, row, &mut acc) } }
                  else { q4k_batched_scalar(x, in_dim, n, row, &mut acc) },
            14 => if avx { unsafe { q6k_batched_avx2(x, in_dim, n, row, &mut acc) } }
                  else { q6k_batched_scalar(x, in_dim, n, row, &mut acc) },
            _ => {
                ...
                for t in 0..n {
                    ...
                    acc[t] = if avx { unsafe { dot_avx2(xt, &w) } } else { dot_scalar(xt, &w) };
                }
            }
        }
        acc
...
fn q4k_row(x: &[f32], row: &[u8]) -> f32 {
    ...
    for b in 0..bpr {
        ...
        for _ in 0..4 {
            ...
            let mut sq1 = 0.0f32; let mut sx1 = 0.0f32;
            for l in 0..32 {
                let xv = xb[o + l];
            ...
            let mut sq2 = 0.0f32; let mut sx2 = 0.0f32;
            for l in 0..32 {
                let xv = xb[o + l];
...
fn gemv_q6_k(x: &[f32], data: &[u8], in_dim: usize, out_dim: usize) -> Vec<f32> {
    ...
    (0..out_dim).into_par_iter().map(|j| {
        let row = &data[j * row_bytes..(j + 1) * row_bytes];
        if avx { unsafe { q6k_row_avx2(x, row) } } else { q6k_row_scalar(x, row) }
    }).collect()
}
...
fn gemv_f32_data(x: &[f32], data: &[u8], in_dim: usize, out_dim: usize) -> Vec<f32> {
    ...
    (0..out_dim).into_par_iter().map(|j| {
        let row = &data[j * row_bytes..(j + 1) * row_bytes];
        (0..in_dim).map(|k| {
            let w = f32::from_le_bytes([row[k*4], row[k*4+1], row[k*4+2], row[k*4+3]]);
            x[k] * w
        }).sum::<f32>()
    }).collect()
}
...
fn dequant_f16(data: &[u8], n: usize) -> Vec<f32> {
    (0..n).map(|i| {
        f16_to_f32(u16::from_le_bytes(data[i*2..i*2+2].try_into().unwrap()))
    }).collect()
}
...
fn dequant_q4_k(data: &[u8], n: usize) -> Vec<f32> {
    ...
    for b in 0..n_blocks {
        ...
        for _ in 0..4 {
            ...
            for l in 0..32 { out.push(d1 * (qs[q_off + l] & 0x0F) as f32 - m1); }
            for l in 0..32 { out.push(d2 * (qs[q_off + l] >> 4)   as f32 - m2); }
