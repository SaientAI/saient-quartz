...
pub fn sigmas(steps: u32, shift: f32) -> Vec<f32> {
    ...
    match steps {
        ...
        n => {
            ...
            let mut out: Vec<f32> = (0..n).map(|i| t_to_sigma(shift, t_max - step * i as f32)).collect();
            out.push(0.0);
            out
        }
    }
...
mod tests {
    ...
    fn schedule_starts_at_one_and_ends_at_zero() {
        for &shift in &[1.0_f32, 3.0, 5.0, 8.0, 12.0] {
            ...
            assert!((s[0] - 1.0).abs() < 1e-6, "shift {shift}: first sigma must be 1.0, got {}", s[0]);
            assert_eq!(*s.last().unwrap(), 0.0, "shift {shift}: schedule must terminate at 0");
        }
    }
        ...
        for &shift in &[1.0_f32, 3.0, 8.0, 12.0] {
            for steps in [1_u32, 2, 4, 8, 12, 20, 50] {
                ...
                for w in s.windows(2) {
                    assert!(w[0] > w[1], "shift {shift} steps {steps}: not decreasing: {w:?}");
                }
            }
        }
    }
    ...
    fn higher_shift_holds_more_noise_through_the_schedule() {
        ...
        let hi = sigmas(8, 8.0);
        for i in 1..7 {
            assert!(hi[i] > lo[i], "step {i}: shift8 {} should exceed shift3 {}", hi[i], lo[i]);
        }
    }
